...
# stock_picker: empty default = ALL stocks (safe for scheduled jobs)
dbutils.widgets.text("stock_picker", "", "Stocks (comma-separated, empty=ALL)")

# start_date: auto-detect from existing table for incremental loads
# If empty or default, compute from MAX(date) in bronze table
dbutils.widgets.text("start_date", "", "Start date (YYYY-MM-DD, empty=incremental)")
...
# Auto-detect start_date for incremental loads
if start_date_input:
    start_date = start_date_input
    print(f"✅ Using explicit start_date: {start_date}")
else:
    # Incremental: start from MAX(date) in existing table minus 5-day buffer
    bronze_table = f"{catalog}.{schema}.stock_prices_bronze"
    try:
        if spark.catalog.tableExists(bronze_table):
            max_dt = spark.sql(f"SELECT MAX(date) AS d FROM {bronze_table}").first()["d"]
            if max_dt:
                # 5-day buffer to catch any late-arriving data / weekend gaps
                start_date = (max_dt - timedelta(days=5)).strftime("%Y-%m-%d")
                print(f"✅ Incremental mode: start_date = {start_date} (MAX(date)={max_dt} minus 5d buffer)")
            else:
                start_date = "2015-01-01"
                print(f"⚠️ Empty bronze table — full backfill from {start_date}")
        else:
            start_date = "2015-01-01"
            print(f"⚠️ Bronze table doesn't exist — full backfill from {start_date}")
    except Exception as e:
        start_date = "2015-01-01"
        print(f"⚠️ Could not detect MAX(date): {e} — full backfill from {start_date}")
...
if stock_data is not None and not stock_data.empty:
    ...
    try:
        ...
        if isinstance(stock_data.columns, pd.MultiIndex):
            ...
            long_df = stock_data.stack(level=symbol_level).reset_index()
...
completeness = (1 - missing_prices / total_prices) * 100 if total_prices != 0 else 0
...
data_date = latest_stock['latest_date'] if latest_stock['latest_date'] is not None else datetime.now().date()
