...
# MAGIC - `gold.attribution_summary`
...
if spark.catalog.tableExists(forecast_tbl) and spark.catalog.tableExists(price_tbl):
    ...
    print("⚠️ Missing stock_forecasts or stock_prices_daily for decision_signals.")

# COMMAND ----------

# 3) Attribution Summary (Top factors + top evidence sources)
evidence_tbl = f"{gold_db}.rag_evidence_log"
attrib_tbl = f"{gold_db}.attribution_summary"

if spark.catalog.tableExists(forecast_tbl) and spark.catalog.tableExists(evidence_tbl):
    forecasts = spark.table(forecast_tbl).filter(
        (F.col("forecast_date") >= F.lit(start_date).cast("date")) &
        (F.col("forecast_date") <= F.lit(end_date).cast("date"))
    )

    evidence = spark.table(evidence_tbl).filter(
        (F.col("as_of_date") >= F.lit(start_date).cast("date")) &
        (F.col("as_of_date") <= F.lit(end_date).cast("date"))
    )

    src_rank = Window.partitionBy("symbol").orderBy(F.col("cnt").desc())
    top_sources = evidence.groupBy("symbol", "source").count() \
        .withColumnRenamed("count", "cnt") \
        .withColumn("rnk", F.row_number().over(src_rank)) \
        .filter(F.col("rnk") <= 3) \
        .groupBy("symbol") \
        .agg(F.collect_list("source").alias("top_sources"))

    attrib = forecasts.join(top_sources, "symbol", "left") \
        .select(
            "symbol",
            "forecast_date",
            "horizon_days",
    ...
    write_partitioned_table(attrib_tbl, attrib, ["forecast_date", "symbol"], "forecast_date", start_date, end_date)
    print(f"✅ Saved attribution_summary: {attrib_tbl}")
else:
    print("⚠️ Missing inputs for attribution_summary.")
