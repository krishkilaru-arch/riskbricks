...
# Widgets — force reset to desired backfill range
try:
    dbutils.widgets.removeAll()
except:
    pass
dbutils.widgets.text("start_date", "2026-02-03", "Start Date (YYYY-MM-DD)")
dbutils.widgets.text("end_date", "2026-04-10", "End Date (YYYY-MM-DD)")
print(f"start_date = {dbutils.widgets.get('start_date')}")
print(f"end_date   = {dbutils.widgets.get('end_date')}")
...
# requests is pre-installed on serverless; skip heavy BigQuery packages to avoid numpy conflicts
...
# Note: schema autoMerge set via table properties, not spark.conf on serverless
...
# Get all symbols from company universe (no filtering)
...
# -------------------------------------------------------------------
# Last 7 days only (Apr 4 – Apr 10, 2026)
# -------------------------------------------------------------------
start_date = datetime(2026, 4, 4)
end_date = datetime(2026, 4, 10)

print(f"📅 GDELT range: {start_date.strftime('%Y-%m-%d')} → {end_date.strftime('%Y-%m-%d')} (7 days)")
...
# MAGIC %md
# MAGIC ### ⚠️ One-Time Rebuild (Run Manually If Needed)
# MAGIC Use this only once to rebuild tables with partitions.
